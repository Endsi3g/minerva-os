...
export const reportsTools = [
  {
    ...
    async handler(_args: Record<string, unknown>) {
      ...
      const [deals, approvals, invoices, clients] = await Promise.all([
        ...
        convexQuery('clients:list', { workspaceId }),
      ]) as [any[], any[], any[], any[]];
      ...
      const topClients = (clients ?? [])
        .map((c: any) => ({
          company: c.company,
          revenue: (invoices ?? []).filter((i: any) => i.clientId === c._id && i.status === 'paid').reduce((s: number, i: any) => s + i.amount, 0),
        }))
        .sort((a: any, b: any) => b.revenue - a.revenue)
        .slice(0, 5);

      return {
        pipelineValue: deals?.reduce((s: number, d: any) => s + d.value, 0) ?? 0,
        winRate,
  ...
  },
  {
    ...
    async handler(_args: Record<string, unknown>) {
      ...
      const suggestions = await convexQuery('agents:getSuggestions', { workspaceId });
  ...
  },
  {
    ...
    async handler(_args: Record<string, unknown>) {
      ...
      (approvals ?? []).forEach((a: any) => {
        if (a.status === 'pending') {
          const days = Math.floor((today.getTime() - new Date(a.submittedDate).getTime()) / 86400000);
          if (days > 5) flags.push({ type: 'approval_stalled', severity: 'medium', message: `Approval "${a.title}" pending for ${days} days` });
        }
      });
